...
public interface ItemRepository extends JpaRepository<Record, Long>, JpaSpecificationExecutor<Record> {
    ...
    @Query(
        ...
        value = """
            ...
            FROM items i
            WHERE (
                LOWER(CAST(i.roles AS varchar)) LIKE '%user%'
                OR i.roles IS NULL
            )
            AND (
                i.codes IS NULL
    ...
    @Query(
        ...
        value = """
            ...
            FROM items i
            WHERE (
                LOWER(CAST(i.roles AS varchar)) LIKE '%admin%'
                OR i.roles IS NULL
            )
            AND (
                i.codes IS NULL
    ...
    @Query(
        ...
        value = """
            ...
            FROM items i
            WHERE (
                LOWER(CAST(i.roles AS varchar)) LIKE '%staff%'
                OR i.roles IS NULL
            )
            AND (
                i.codes IS NULL
